...
void triwild::feature::preprocessing(Eigen::MatrixXd& V, std::vector<std::array<int, 2>>& edges){
...
//    }
    ...
    for(int i=0;i<features.size();i++) {
        //only for check
        {
            ...
            Point_2f p0(V(features[i]->v_ids.front(), 0), V(features[i]->v_ids.front(), 1));
            if (p != p0) {
                cout << "feature " << i << " ";
        ...
        }
        {
            ...
            Point_2f p0(V(features[i]->v_ids.back(), 0), V(features[i]->v_ids.back(), 1));
            if (p != p0) {
                cout << "feature " << i << " ";
                cout << features[i]->type << endl;
                cout << std::setprecision(16) << p << endl << p0 << endl;
...
void triwild::feature::simplify(){
    ...
    for (auto &feature:features) {
        ...
        Point_2f end = feature->eval(feature->paras.back());//TODO: would have gaps!!!
    ...
    cout << simplified_cnt << "/"<<features.size()<<" features are simplified" << endl;
}

void triwild::feature::get_inflections(std::vector<std::vector<double>>& inflections) {
    inflections.clear();
...
void triwild::feature::cut_reflex(std::vector<std::vector<double>>& inflections) {//cut loop
...
//        std::vector<bool> is_inflection;//not used
        ...
        for (int i = 0; i < ranges.size() - 1; i++) {
            double turning_angle = features[feature_id]->how_curve(ranges[i], ranges[i + 1]);
            if (turning_angle < 179)
                continue;
            double t = features[feature_id]->get_cut_t(ranges[i], ranges[i + 1], (turning_angle >= 180));
...
//            samples[i] = {features[i]->eval(features[i]->paras.front()), features[i]->eval(features[i]->paras.back())};
//            continue;
//        }

        auto &feature_samples = samples[i];
        feature_samples = {features[i]->eval(features[i]->paras.front()), features[i]->eval(features[i]->paras.back())};
        ...
        while (true) {
            ...
            for (int j = 0; j < feature_samples.size() - 1; j++) {
                ...
                feature_samples.insert(feature_samples.begin() + j + 1, features[i]->eval(t));
...
void triwild::feature::remove_short_features(std::vector<std::vector<double>>& inflections, std::vector<std::vector<Point_2f>>& samples,
                                              std::vector<std::vector<double>>& ts) {
...
//                || features[i]->degree == features[id]->degree && i > id)
            if (samples[i].size() > samples[id].size()
                || (samples[i].size() == samples[id].size() && i > id))
                feature_pairs.push_back({{i, (int) id}});
        }
...
//             }
//             if (boxes[pair[0]][1].x < boxes[pair[1]][0].x || boxes[pair[0]][0].x > boxes[pair[1]][1].x
//                 || boxes[pair[0]][1].y < boxes[pair[1]][0].y || boxes[pair[0]][0].y > boxes[pair[1]][1].y)
//                 continue;
//
...
//     }
    ...
    cout << "#check_feature = " << std::count(is_used.begin(), is_used.end(), true) << "(" << features.size() << ")"
         << endl;
    ...
    std::vector<std::vector<bool>> v_is_removed(features.size(), std::vector<bool>());
    for (int i = 0; i < features.size(); i++)
...
//            cout<<"is_corner 1"<<endl;
            ...
            if (samples[feature_id].front() == samples[other_feature_id].front())
                ...
                v2 = samples[other_feature_id][samples[other_feature_id].size() - 2] - samples[other_feature_id].back();
            else
                is_corner = false;
...
//                    cout << "---------" << endl;
                    ...
                    if (is_valid) {
                        for (int j = 0; j < cur_v_is_removed.size(); j++) {
//                        cout<<j<<" "<<cur_v_is_removed[j]<<" "<<v_is_removed[feature_id][j]<<endl;
...
//        } else if(cur_v_is_removed.back() && !v_is_removed[feature_id].back()) {
        } else if (cur_v_is_removed.back()) {
//            cout<<"is_corner 2"<<endl;
            Vector_2f v1 = samples[feature_id][samples[feature_id].size() - 2] - samples[feature_id].back();
            Vector_2f v2;
...
//                    optimization::pausee();
        ...
        for (int i = 0; i < cur_v_is_removed.size(); i++) {
            v_is_removed[feature_id][i] = v_is_removed[feature_id][i] || cur_v_is_removed[i];
        }
        ...
        for (int i = 0; i < samples[feature_id].size(); i++) {
            if (i == 0) {
                ...
                if (!v_is_removed[feature_id][i] && v_is_removed[feature_id][i + 1] && v_is_removed[feature_id][i - 1])
                    v_is_removed[feature_id][i] = true;
            }
        }
        if (std::count(v_is_removed[feature_id].begin(), v_is_removed[feature_id].end(), true) ==
            v_is_removed[feature_id].size()) {
            f_is_removed[feature_id] = true;
            continue;
        }
        ...
        f_mesh.vertices.clear();
        f_mesh.vertices.create_vertices(samples[feature_id].size());//the old vertices has been re-ordered!!!!!!!!!
        for (int i = 0; i < samples[feature_id].size(); i++) {
            GEO::vec3 &p = f_mesh.vertices.point(i);
            p[0] = samples[feature_id][i].x;
            p[1] = samples[feature_id][i].y;
...
//            if(all[j]==i)
//                continue;
//            Point_2f p1 = features[all[j]]->eval(features[all[j]]->paras.front());
//            Point_2f p2 = features[all[j]]->eval(features[all[j]]->paras.back());
//            double l11 = (p_1-p1).length();
//            double l12 = (p_1-p2).length();
//            double l21 = (p_2-p1).length();
//            double l22 = (p_2-p2).length();
//            if(l11<feature_eps || l12<feature_eps || l21<feature_eps || l22<feature_eps){
//                cout<<"feature "<<features[i]->curve_id<<" "<<features[all[j]]->curve_id<<endl;
...
void triwild::feature::remove_high_curvature(std::vector<std::vector<double>>& inflections, std::vector<std::vector<Point_2f>>& samples,
                                              std::vector<std::vector<double>>& ts) {
    int cnt_removed = 0;
    for (int feature_id = 0; feature_id < features.size(); feature_id++) {
        if (features[feature_id]->type == "Line")
        ...
        for (int i = 0; i < samples[feature_id].size() - 1; i++) {
            ...
            double angle = features[feature_id]->how_curve(ts[feature_id][i], ts[feature_id][i + 1]);
...
//                ranges.push_back(j);
        ...
        for (int j = 0; j < removed_ranges.size(); j += 2) {
            int new_feature_id = push_back_new_secondary_feature(features[feature_id]);
...
void triwild::feature::cut_inflections(std::vector<std::vector<double>>& inflections, std::vector<std::vector<Point_2f>>& samples,
                                        std::vector<std::vector<double>>& ts) {
    const int features_size = features.size();
    for (int feature_id = 0; feature_id < features_size; feature_id++) {
...
//        auto& old_inflection = inflections[feature_id];
        for (int j = 0; j < ranges.size() - 1; j++) {
            if (j == ranges.size() - 2) {
                ...
                int new_feature_id = push_back_new_feature(features[feature_id]);
                features[new_feature_id]->paras = {ts[feature_id][ranges[j]], ts[feature_id][ranges[j + 1]]};
                samples.push_back(std::vector<Point_2f>(samples[feature_id].begin() + ranges[j],
                                                        samples[feature_id].begin() + ranges[j + 1] + 1));
                ts.push_back(std::vector<double>(ts[feature_id].begin() + ranges[j],
                                                 ts[feature_id].begin() + ranges[j + 1] + 1));
                inflections.emplace_back();
...
//        std::vector<double> ranges = inflections[feature_id];
//        ranges.insert(ranges.begin(), features[feature_id]->paras.front());
...
//            } else {
//                int new_feature_id = push_back_new_feature(features[feature_id]);
//                features[new_feature_id]->paras = {ranges[j], ranges[j + 1]};
////                samples.push_back(
////                        std::vector<Point_2f>(samples[feature_id].begin() + ranges[j],
////                                              samples[feature_id].begin() + ranges[j + 1] + 1));
////                ts.push_back(std::vector<double>(ts[feature_id].begin() + ranges[j],
...
void triwild::feature::gen_segments(Eigen::MatrixXd& V, std::vector<std::array<int, 2>>& edges) {
    double dd = args.feature_epsilon * args.diagonal_len * 10;
    double dd_2 = dd * dd;

    double min_len_2 = (args.min_edge_length * args.target_edge_len) * (args.min_edge_length * args.target_edge_len);

    std::vector<std::vector<Point_2f>> samples(features.size());
    std::vector<std::vector<Point_2f>> secondary_samples(secondary_features.size());
    int cnt_v = 0;
    auto sampling = [&](std::vector<std::shared_ptr<FeatureElement>>& features, std::vector<std::vector<Point_2f>>& samples) {
        for (int i = 0; i < features.size(); i++) {
            samples[i] = {features[i]->eval(features[i]->paras.front()), features[i]->eval(features[i]->paras.back())};
            if (features[i]->type == "Line") {
            ...
            while (true) {
                ...
                for (int j = 0; j < is_qualified.size(); j++) {
                    ...
                    if (l_2 <= dd_2 &&
                        features[i]->how_curve(features[i]->paras[j], features[i]->paras[j + 1]) <=
...
int triwild::feature::push_back_new_feature(std::shared_ptr<FeatureElement> old_feature) {
...
int triwild::feature::push_back_new_secondary_feature(std::shared_ptr<FeatureElement> old_feature) {
    if (old_feature->type == "Line")
...
void triwild::feature::check(std::vector<std::vector<double>>& inflections, std::vector<std::vector<Point_2f>>& samples,
    ...
    for(int i=0;i<features.size();i++) {
        ...
        if (features[i]->paras.back() <= features[i]->paras.front()) {
            ...
            cout<<"feature "<<i<<endl;
            cout<<features[i]->paras.front()<<" "<<features[i]->paras.back()<<endl;
            optimization::pausee();
        }
    ...
    for(int i=0;i<secondary_features.size();i++) {
        ...
        if (secondary_features[i]->paras.back() <= secondary_features[i]->paras.front()) {
            cout << "secondary_features[i]->paras.back()<=secondary_features[i]->paras.front()" << endl;
            optimization::pausee();
        }
    }
}
